...
loglogdensity_plot <- function(beta, knots, show_knots = TRUE, show_points = FALSE, gpModel = NULL, beta_true = NULL, beta_init = NULL, err_bounds = NULL)
{
  ...
  if (show_points) {
    lg_labs <- c(lg_labs, 'true points')
    lg_cols <- c(lg_cols, 'grey50')
    lg_ltys <- c(lg_ltys, NA)
    lg_lwds <- c(lg_lwds, NA)
    lg_pchs <- c(lg_pchs, 16)
    graphics::points(xx, pts, pch = 20, cex = 1.5, col = 'grey50')
  }
  graphics::lines(logx, l1, lwd = 2, col = 'blue')
  if (!is.null(beta_true)) {
    lg_labs <- c(lg_labs, 'closest fit')
    lg_cols <- c(lg_cols, 'black')
    lg_ltys <- c(lg_ltys, 2)
    lg_lwds <- c(lg_lwds, 2)
    lg_pchs <- c(lg_pchs, NA)
    graphics::lines(logx, l2, lwd = 2, lty = 2)
  }
  if (!is.null(beta_init)) {
    lg_labs <- c(lg_labs, 'initial')
    lg_cols <- c(lg_cols, 'orange')
    lg_ltys <- c(lg_ltys, 2)
    lg_lwds <- c(lg_lwds, 2)
    lg_pchs <- c(lg_pchs, NA)
    graphics::lines(logx, l3, lty = 2, lwd = 2, col = 'orange')
  }
...
logdensity_plot <- function(beta, knots, show_knots = TRUE, show_points = FALSE, gpModel = NULL, beta_true = NULL, beta_init = NULL, err_bounds = NULL)
{
  ...
  if (!is.null(beta_true)) {
    ...
    lg_lwds <- c(lg_lwds, 2)
    lg_pchs <- c(lg_pchs, NA)
    graphics::lines(logx, l2, lty = 2, lwd = 2)
  }
  if (!is.null(beta_init)) {
    lg_labs <- c(lg_labs, 'initial')
    lg_cols <- c(lg_cols, 'orange')
...
covar_plot <- function(h, beta, knots, gpModel = NULL, beta_true = NULL, beta_init = NULL, err_bounds = NULL)
{
  ...
  if (!is.null(beta_true)) {
    lg_labs <- c(lg_labs, 'optimal')
    lg_cols <- c(lg_cols, 'black')
    lg_ltys <- c(lg_ltys, 2)
    graphics::lines(h, l2, lwd = 2, lty = 2)
  }
  if (!is.null(gpModel)) {
    lg_labs <- c(lg_labs, 'true')
    lg_cols <- c(lg_cols, 'orange')
    lg_ltys <- c(lg_ltys, 2)
    graphics::lines(h, l3, col = 'orange', lwd = 2, lty = 2)
  }
  graphics::abline(h = 0, lty = 3)
  graphics::legend('topright', lg_labs, col = lg_cols, lwd = 2, lty = lg_ltys)
